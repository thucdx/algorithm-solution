...
int main( int argc, char* argv[] ) {
	...
	while(scanf("%s", a)!=EOF){
		...
		if(s.find('_')!=-1){
			bool ok=s.find("__")==-1;
			reps(i,s)ok&=islower(s[i])||s[i]=='_';
			if(!ok||s[0]=='_'||*(s.end()-1)=='_'){
				s="Error!";
			} else {
				vs x=split<string>(s,"_");
				s=x[0];
				fr(i,1,sz(x)-1)x[i][0]=toupper(x[i][0]),s+=x[i];
			}
		} else {
			if(isupper(s[0]))s="Error!";
			else frd(i,sz(s)-1,0)if(isupper(s[i]))s[i]=tolower(s[i]),s.insert(i,1,'_');
		}
