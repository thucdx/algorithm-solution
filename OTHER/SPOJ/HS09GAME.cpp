...
int grundyNumber(int pos) {
	set<int> s;
	rep(i,3)if(pos-moves[i]>=0)s.insert(grundyNumber(pos-moves[i]));
	int ret=0;
...
int main( int argc, char* argv[] ) {
	//rep(i,20)cout<<grundyNumber(i)<<" ";
	int grundy[]={0,0,1,1,2,2,3};
	...
	for(int testCase=1;testCase<=testCases;testCase++){
		...
		if (res==0)printf("Robert wins.\n\n");
		else {
			int move=-1,heap=-1;
			frd(j,2,0){
				rep(i,n){
					if(a[i]>=moves[j]){
						int r=res^grundy[a[i]%7]^grundy[(a[i]-moves[j])%7];
						if(r==0){
							heap=i;
							move=moves[j];
							goto m1;
