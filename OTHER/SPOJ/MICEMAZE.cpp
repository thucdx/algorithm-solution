...
void dijkstra(Graph g, int s, vector<long long> &prio, vector<int> &pred) {
	typedef pair<long long, int> qitem;
	long long INF = 1234567890123456789LL;
	int n = g.size();
	prio.assign(n, INF);
	pred.assign(n, -1);
	prio[s] = 0;
...
int main( int argc, char* argv[] ) {
	...
	vl prio;
	vi pred;
	dijkstra(g,e,prio,pred);

	int res=0;
	reps(i,prio)if(prio[i]<=t)++res;
