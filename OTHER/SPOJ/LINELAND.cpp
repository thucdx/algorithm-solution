...
int main( int argc, char* argv[] ) {
	...
	for(int testCase=1;testCase<=testCases;testCase++){
		...
		vd h1(10001),h2(10001);
		vi x(n),y(n);

		rep(i,n){
			cin>>x[i]>>y[i];
			if(i>0){
				fr(j,x[i-1],x[i]-1)h1[j]=y[i-1]+double(j-x[i-1])*(y[i]-y[i-1])/(x[i]-x[i-1]);
				fr(j,x[i-1],x[i]-1)h2[j]=y[i-1]+double(j-x[i-1]+1)*(y[i]-y[i-1])/(x[i]-x[i-1]);
			}			
		}
		...
		fr(i,x[0],x.back()-1){
			sum+=(h1[i]+h2[i])/2;
			double h=min(h1[i],h2[i]);

			while (!q.empty() && q.back().first > h - eps)	q.pop_back();
			q.pb(mp(h,i));

			if(i-x[0]+1>=L){
				double minh=q.front().first;
				checkmin(res,sum-L*minh);
				sum-=(h1[i-L+1]+h2[i-L+1])/2;
				if (!q.empty() && abs(q.front().first - min(h1[i-L+1],h2[i-L+1]))<eps && q.front().second==i-L+1) q.pop_front();
			}
		}
