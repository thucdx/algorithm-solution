...
pair<int,pii> go(int r,int c){
	...
	while(!q.empty()){
		...
		rep(i,4){
			...
			if(!inside(nr,nc,R,C)||a[nr][nc]!='.')continue;
			q.push(d+1);
			q.push(nr);
			q.push(nc);
			q.push(r);
	...
	for(int testCase=1;testCase<=testCases;testCase++){		
		...
		rep(i,R){
			...
			rep(j,C)if(a[i][j]=='.'){
				...
				checkmax(res,d[i][j+1]+d[i+1][j]+1);
			}
		}
		*/
...
m1:
		pair<int,pii> p2 = go(p1.second.first,p1.second.second);
