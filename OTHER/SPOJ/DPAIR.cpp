...
/* ... :" We are I/O Accelerator ... Use us at your own risk <img src="http://www.shuizilong.com/house/wp-includes/images/smilies/icon_wink.gif" alt=";)" class="wp-smiley">  ... " .. */
...
const int N = 800001, M = 200001, U = 1000001;
int A[N], B[N], C[U];
int l[M], r[M]; LL res[M];
int n, m;
...
int main(){
    ...
    REP_C(i, _RD(n)) RD(A[i]); REP_C(i, _RD(m)) RD(l[i], r[i]);
 
    int p = 0; REP(i, n){
        for(;p<n&&!C[A[p]];++p) ++C[A[p]]; --C[A[i]];
        REP(j, m) res[j] += max(0, min(r[j], len) - l[j] + 1);
    }
