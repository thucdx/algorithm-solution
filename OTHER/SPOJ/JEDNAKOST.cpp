...
int main( int argc, char* argv[] ) {	
	...
	frd(i,sz(a)-1,4)if(a[i]=='0'&&a[i-1]=='0'&&a[i-2]=='0'&&a[i-3]=='0'&&a[i-4]=='0'){
		a.erase(i,1);
	}

	vvi d(sz(a)+1,vi(b+1,10000));
	d[0][0]=0;

	fr(i,1,sz(a)){
		fr(j,max(0,i-18),i-1){
			ll v=cast<ll>(a.substr(j,i-j));
			for(ll k=v;k<=b;k++){
				checkmin(d[i][k],d[j][k-v]+1);
			}
		}
	}

	cout<<d[sz(a)][b]-1<<endl;
