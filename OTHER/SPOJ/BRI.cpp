...
double solve(double x1,double x2){
	return s1*(hyp(a,x1)+hyp(b,c-x2)) + s2*hyp(h,x2-x1);
}
...
int main( int argc, char* argv[] ) {
	...
	*/
	...
	for(int testCase=1;testCase<=testCases;testCase++){
		...
		double x2=c;
		double L=c/2.;
		double res=solve(x1,x2);
		while(L>1e-5){
			double tmp;
			bool find=false;
			tmp=solve(x1+L,x2); if(tmp<res) res=tmp,x1+=L,find=true;
			else { tmp=solve(x1-L,x2); if(tmp<res) res=tmp,x1-=L,find=true; }
			tmp=solve(x1,x2+L); if(tmp<res) res=tmp,x2+=L,find=true;
			else { tmp=solve(x1,x2-L); if(tmp<res) res=tmp,x2-=L,find=true; }
			if(!find)L/=2;
		}
