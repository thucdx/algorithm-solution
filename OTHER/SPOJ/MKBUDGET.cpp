...
int main( int argc, char* argv[] ) {
	...
	while(1){		
		int n,hiring,salary,severance;
		scanf("%d%d%d%d", &n,&hiring,&salary,&severance);
		if(n==0)break;
		vi a(n);
		rep(i,n)scanf("%d",&a[i]);
		a.insert(a.begin(),0);		

		vvl d(n+1,vl(n+1,LONG_MAX));
		d[0][0]=0;
		
		rep(i,n)rep(j,n+1)if(d[i][j]!=LONG_MAX){
			rep(k,n)if(a[k+1]>=a[i+1]){
				ll cost=a[k+1]*salary;
				ll cnt=a[k+1]-a[j];
				if(cnt>0)cost+=cnt*hiring;
				else cost+=-cnt*severance;
				checkmin(d[i+1][k+1],d[i][j]+cost);
			}				
		}

		ll res=LONG_MAX;
		rep(i,n+1)checkmin(res,d[n][i]);

		printf("Case %d, cost = $%ld\n", ++testcase, res);
	}
