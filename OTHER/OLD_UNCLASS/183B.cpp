// Created : 11:05:57 PM Oct 20, 2012
// Task    : 183B
...
bool getLine(int xa, int ya, int xb, int yb, ll& a, ll& b, ll& c){
	// ax + by + c = 0 , y = 0 => ax + c = 0;
...
inline void run() {
	...
	FOR(i, 0, m - 1){
		FOR(j, i + 1, m - 1){
			// cat truc ox tai t nguyen thuoc [1, m]
			ll a, b, c;
			if (!getLine(x[i], y[i], x[j], y[j], a, b, c))
			{
					continue;
			}
			else {
